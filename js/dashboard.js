...
(function () {
  ...
  // Données dérivées
  const arrivals = RESERVATIONS.filter(r => r.arrivee === AUJOURDHUI && r.canal !== 'bloque');
  const departures = RESERVATIONS.filter(r => r.depart === AUJOURDHUI && r.canal !== 'bloque');
  const last = STATS.occupation.length - 1;
  const occ = STATS.occupation[last], occPrev = STATS.occupation[last - 1];
  const ca = STATS.ca[last], caPrev = STATS.ca[last - 1];
  const caTrend = Math.round((ca - caPrev) / caPrev * 100);
  const unread = Layout.unreadCount();
  ...
  const KPIS = [
    ...
    { label: 'Taux d\'occupation', value: occ + ' %', foot: `<span class="trend trend--up">▲ ${occ - occPrev} pts</span> ce mois-ci`, ic: icon('<path d="M3 3v18h18"/><path d="M7 14l4-4 3 3 5-6"/>') },
    { label: 'CA du mois', value: formatEuro(ca), foot: `<span class="trend trend--up">▲ ${caTrend} %</span> vs. mois dernier`, ic: icon('<path d="M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>') },
    { label: 'Messages non lus', value: unread, foot: `${CONVERSATIONS.filter(c => c.nonLu > 0).length} conversations à traiter`, ic: icon('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>') },
  ...
  const next = RESERVATIONS
    .filter(r => r.canal !== 'bloque' && parseDate(r.arrivee) >= parseDate(AUJOURDHUI))
    .sort((a, b) => parseDate(a.arrivee) - parseDate(b.arrivee))
    .slice(0, 7);
  document.getElementById('dash-next').innerHTML = next.map(r => {
    const l = getLogement(r.logementId);
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: STATS.occupation, borderColor: tok('--blue-600'), backgroundColor: grad, fill: true,
